...
CREATE OR REPLACE FUNCTION public.is_trainer() RETURNS boolean AS $$
...
CREATE POLICY "Owner ALL profiles" ON profiles FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL trainers" ON trainers FOR ALL TO authenticated USING (public.is_owner());
...
-- Member: Read/Write own.
CREATE POLICY "Owner ALL members" ON members FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL member_trainers" ON member_trainers FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Trainer ALL own member_trainers" ON member_trainers FOR ALL TO authenticated USING (
    public.is_trainer() AND trainer_id IN (SELECT id FROM trainers WHERE profile_id = auth.uid())
);
CREATE POLICY "Member read own member_trainers" ON member_trainers FOR SELECT TO authenticated USING (
    public.is_member() AND member_id IN (SELECT id FROM members WHERE profile_id = auth.uid())
);

-- Membership_Plans:
-- Owner: ALL
-- Others: Read-only
CREATE POLICY "Owner ALL membership_plans" ON membership_plans FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL memberships" ON memberships FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Trainer read assigned members memberships" ON memberships FOR SELECT TO authenticated USING (
    public.is_trainer() AND member_id IN (
        SELECT member_id FROM member_trainers mt JOIN trainers t ON mt.trainer_id = t.id WHERE t.profile_id = auth.uid()
    )
);
...
CREATE POLICY "Owner ALL payments" ON payments FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL assessments" ON assessments FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL diet_plans" ON diet_plans FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL workout_plans" ON workout_plans FOR ALL TO authenticated USING (public.is_owner());
...
-- Member: Read/Write own
CREATE POLICY "Owner ALL member_diet_plans" ON member_diet_plans FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL member_workout_plans" ON member_workout_plans FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL exercises" ON exercises FOR ALL TO authenticated USING (public.is_owner());
...
-- Member: Read
CREATE POLICY "Owner ALL group_activities" ON group_activities FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Trainer ALL own group_activities" ON group_activities FOR ALL TO authenticated USING (
    public.is_trainer() AND trainer_id IN (SELECT id FROM trainers WHERE profile_id = auth.uid())
);
CREATE POLICY "Trainer read group_activities" ON group_activities FOR SELECT TO authenticated USING (public.is_trainer());
CREATE POLICY "Member read group_activities" ON group_activities FOR SELECT TO authenticated USING (public.is_member());

-- Activity Bookings:
-- Owner: ALL
-- Trainer: Read for their activities
-- Member: Read/Write own
CREATE POLICY "Owner ALL activity_bookings" ON activity_bookings FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Trainer read own activity_bookings" ON activity_bookings FOR SELECT TO authenticated USING (
    public.is_trainer() AND activity_id IN (SELECT id FROM group_activities WHERE trainer_id IN (SELECT id FROM trainers WHERE profile_id = auth.uid()))
);
...
-- Member: Read/Write own
CREATE POLICY "Owner ALL attendance" ON attendance FOR ALL TO authenticated USING (public.is_owner());
...
CREATE POLICY "Owner ALL products" ON products FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Read products" ON products FOR SELECT TO authenticated USING (true);

CREATE POLICY "Owner ALL store_sales" ON store_sales FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Member read own store_sales" ON store_sales FOR SELECT TO authenticated USING (
    public.is_member() AND member_id IN (SELECT id FROM members WHERE profile_id = auth.uid())
);

CREATE POLICY "Owner ALL store_sale_items" ON store_sale_items FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Member read own store_sale_items" ON store_sale_items FOR SELECT TO authenticated USING (
    public.is_member() AND sale_id IN (SELECT id FROM store_sales WHERE member_id IN (SELECT id FROM members WHERE profile_id = auth.uid()))
);

-- Leads:
-- Owner: ALL
-- Trainer: Read/Write assigned to them
CREATE POLICY "Owner ALL leads" ON leads FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Trainer ALL assigned leads" ON leads FOR ALL TO authenticated USING (
    public.is_trainer() AND assigned_trainer IN (SELECT id FROM trainers WHERE profile_id = auth.uid())
);

-- Notifications:
-- Owner: ALL
-- Member/Trainer: Read/Write own
CREATE POLICY "Owner ALL notifications" ON notifications FOR ALL TO authenticated USING (public.is_owner());
CREATE POLICY "Users ALL own notifications" ON notifications FOR ALL TO authenticated USING (recipient_profile_id = auth.uid());

-- Audit Logs:
-- Owner: ALL
CREATE POLICY "Owner ALL audit_logs" ON audit_logs FOR ALL TO authenticated USING (public.is_owner());

-- Integration Sources:
-- Owner: ALL
CREATE POLICY "Owner ALL integration_sources" ON integration_sources FOR ALL TO authenticated USING (public.is_owner());
