-- 1. Partial unique index on attendance: one check-in per member per day.
-- Prevents duplicate attendance from concurrent requests (race condition).
CREATE UNIQUE INDEX IF NOT EXISTS idx_attendance_member_day_unique
ON public.attendance (member_id, (occurred_at::date))
WHERE check_in = true;

-- 2. Fix submit_website_lead: re-raise audit log errors instead of silent swallow.
-- The EXCEPTION WHEN OTHERS THEN NULL block masked audit failures from anon callers.
-- Now logs the error to the system but still completes the lead creation.
-- Note: We cannot modify the existing function without a full replacement since it
-- is SECURITY DEFINER. The fix is to use PERFORM (which raises on failure) and
-- catch only specific expected errors (e.g., anon role lacking INSERT on audit_logs).
-- Since the function already uses SECURITY DEFINER, insert_audit_log should succeed.
-- The safest fix: remove the blanket EXCEPTION handler and let real errors propagate.
...
BEGIN
  ...
  -- Audit log: SECURITY DEFINER insert_audit_log should always succeed.
  -- If it fails, let the error propagate (do not silently swallow).
...
COMMENT ON FUNCTION public.submit_website_lead(text, text, text, text, text, text) IS
  'Public website early-access capture. Forces source=Website and stage=new. Audit errors now propagate instead of being silently swallowed (migration 000023).';
