              ...
              <select name="membership_id" required className="mt-1 block w-full border border-gray-300 rounded p-2">
                ...
                {membersWithMemberships?.map(m =>
                  m.memberships?.map((ms) => {
                    const membershipPlan = Array.isArray(ms.membership_plans) ? ms.membership_plans[0] : ms.membership_plans;
                    return (
                      <option key={ms.id} value={ms.id}>{m.name} - {membershipPlan?.name} (${ms.pending_amount} pending)</option>
                    );
                  })
                )}
              </select>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {payments?.map((payment) => {
                  ...
                  const membershipPlan = membership ? (Array.isArray(membership.membership_plans) ? membership.membership_plans[0] : membership.membership_plans) : null;
                  ...
                  <tr key={payment.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">{membershipPlan?.name}</div>
                    </td>
